...
AddonID ='script.ChainsMaintenance'
...
# ######################### CATEGORIES ################################
def CATEGORIES():
    CreateDir('[COLOR white][B]FRESH START[/B][/COLOR]','url','fresh_start',ADDON_ICON,ADDON_FANART,'')
    CreateDir('[COLOR white][B]MY WIZARD[/B][/COLOR]','ur','builds',ADDON_ICON,ADDON_FANART,'', isFolder=True)
...
def FRESHSTART(mode='verbose'):
    if mode != 'silent': select = xbmcgui.Dialog().yesno("Chains Maintenance", 'Are you absolutely certain you want to wipe this install?' + '\n' + 'All addons EXCLUDING THIS WIZARD will be completely wiped!', yeslabel='Yes',nolabel='No')
    else: select = 1
    if select == 0: return
    elif select == 1:
...
if action   == None: CATEGORIES()
    ...
    xbmc.executebuiltin('Runscript("special://home/addons/script.ChainsMaintenance/resources/lib/modules/speedtest.py")')

...
AddonID = 'plugin.video.playlistloader_mod'
                                ...
                                for start,stop,title in epgDict.get('prg').get(epgDict[u'data'][idx][0]):
                                    ...
                                    if stime <= dnow <= etime or next:
                                        ...
                                        if use_time == 'true':
                                            stmp = '%s-%s' % (ebgn, eend)
                                            t2len += (len(stmp) + 1) 
                                            if not next: title2nd += ' [COLORdeepskyblue]%s[/COLOR]' % stmp
                                        ...
                                        if not next:
                                            plot += '[B][COLORdeepskyblue]%s-%s[/COLOR]\n[COLORwhite]%s[/COLOR][/B]' % (ebgn, eend, title)
                                            name = '[B]%s[/B]\n%s' % (name.ljust(int(t2len * 1.65)), title2nd)
                                            next = True
                                        else: 
                                            plot += '\n\n[B][COLORdeepskyblue]%s-%s[/COLOR]\n%s[/B]' % (ebgn, eend, title)
                                            next = False
                                            break
